...
class Test_Requestlistmodel extends CI_Model{
    ...
    function test_request_list_get_assigned($test_request_id,$user_type_id,$department_id,$user_id){
         $userid=$user_id;
        ...
        if($utid==6||$utid==7){
            ...
            $sql="SELECT * FROM test_request
            JOIN user ON user.test_request_id=test_request.id
            JOIN assignment ON assignment.test_request_id=user.test_request_id where assignment.analyst_assigned_id=$userid  AND assignment.test_request_id=user.test_request_id  AND assignment.analyst_name=user.username AND user.test_request_id=$trid AND user.department_id=$dp_id";
            $query=$this->db->query($sql);
            return $query->result();

        }elseif($utid==5 && $dp_id==3){
            $sql="SELECT * FROM test_request
            JOIN assignment ON assignment.test_request_id=test_request.id where assignment.analyst_assigned_id= $userid";
            $query=$this->db->query($sql);
            return $query->result();
     ...
     function test_request_list_get_unassigned($test_request_id,$user_type_id,$department_id,$user_id){
         $userid=$user_id;
         $trid = $test_request_id;
         $utid = $user_type_id;
         $dp_id =$department_id;
