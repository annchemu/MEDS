...
class Report extends CI_Controller {
	...
	function index() {
		$sql = "SELECT DATE_FORMAT(ht_date,'%m/%d/%x') as period
				FROM humiditytemp 
				GROUP BY DATE_FORMAT(ht_date,'%m/%d/%x')
				ORDER BY DATE_FORMAT(ht_date,'%m/%d/%x') asc";
		...
		foreach ($weeks as $week) {
			...
			$sql = "SELECT ht_location as location,min_temp as total
				    FROM humiditytemp 
				    WHERE DATE_FORMAT(ht_date,'%m/%d/%x')='$week'
