<?php
class Ph_Alkalinity extends CI_Controller {
    ...
    $data['ph_alkalinity']=
    $this->db->select('*')->get_where('ph_alkalinity', array('test_request_id' => $test_request_id))->result_array();
    ...
    $this->load->view('ph_alkalinity_monograph_view',$data);
    $this->load->helper(array('form'));
    }
    function ph_alkalinity_monograph() {
    ...
    $data['sql']=
    $this->db->select('*')->get_where('ph_alkalinity_monograph', array('test_request_id' => $test_request_id))->result_array();
    ...
    $this->load->view('view_ph_alkalinity_monograph',$data);
    $this->load->helper(array('form'));
    }
    ...
    $query=$this->db->select('test_request.id AS tr,test_request.client_id,test_request.active_ingredients,test_request.manufacturer_name,test_request.manufacturer_address,test_request.batch_lot_number,
    test_request.sample_source,test_request.expiry_date,test_request.date_manufactured,test_request.quantity_type,test_request.sample_source,test_request.laboratory_number,test_request.applicant_name,test_request.reference_number,
    ...
    $this->load->view('ph_alkalinity_view',$data);
    $this->load->helper(array('form'));
    }
    function save(){
	   $this->load->model('ph_alkalinity_model');

        if ($this->input->post('submit')) {
            $this->ph_alkalinity_model->process();
        }                   
	}
	function save_monograph(){
       $this->load->model('ph_alkalinity_model');
        ...
        if ($this->input->post('submit')) {
            $this->ph_alkalinity_model->process_monograph();
        }                
    }
